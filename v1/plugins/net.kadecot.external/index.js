...
const AUTO_CLOUD_RECONNECT_INTERVAL = 3000 ;
					...
					clsock.on('connect', function(conn) {
						...
						conn.on('close',function(){
							...
							Promise.all(unsubPromises).then( ()=>{
								...
								// Even if the cloud is disconnected, there is no need to close wamp connection
							...
							setTimeout(()=>{
								...
								cloud_connect_main() ;
							},AUTO_CLOUD_RECONNECT_INTERVAL) ;
						}) ;

					});

					clsock.on('connectFailed',()=>{
						setTimeout(()=>{
							//console.log('Trying reconnection to '+client.host) ;
							cloud_connect_main() ;
						},AUTO_CLOUD_RECONNECT_INTERVAL) ;
					}) ;
