...
function log(msg){ console.log(REALM+':main: '+msg); }
//////////////////////////////////////
// Load libraries
var autobahn , when ;
try {
...
function register_plugin( session_id , prefix ){
	plugins[prefix] = {session_id:session_id} ;
...
function unregister_plugin( session_id ){
	...
	if( prefix === undefined ){return ;}	// the session is not plugin.
	...
	connection.onopen = function (session){
		...
		var dl = [
			...
			, session.register(PREFIX+'.procedure.registerplugin', function(args, kwargs, details){
				...
				var session_id = args[0] , prefix = args[1] ;
			...
			, session.register(PREFIX+'.procedure.registerdevice', function(args, kwargs, details){
				...
				if( devices[key] != undefined ){
					return {success:true , deviceId:devices[key].deviceId} ;
				}
				d.deviceId = deviceid_count++ ;
				d.status = true ;
				devices[key] = d ;
				return {success:true,deviceId:d.deviceId} ;
			})
