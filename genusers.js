...
var bCloudMode = false , bBridgeMode = false ;

if( process.argv.length < 3 ){
	console.error('Usage : node genusers.js #user [cloud] [bridge]') ;
	console.error('cloud mode will randomize default user password') ;
	console.error('bridge mode will remove all plugins and randomize default user password') ;

	console.error('Example 1 : node genusers.js 5  => Generate 5 users') ;
	console.error('Example 2 : node genusers.js 100 cloud  => Generate 100 users and configure in cloud mode') ;
	console.error('Example 3 : node genusers.js 10 cloud bridge  => Generate 10 users and configure in cloud/bridge mode') ;
	process.exit(-1) ;
}
...
for( var aai=2;aai<process.argv.length;++aai ){
	...
	else if(process.argv[aai] == 'cloud')	bCloudMode = true ;
...
for( var i=0 ; i<=user_num ; ++i ){
	...
	if( i==0 )	{ username = 'user' ; if( !bCloudMode&&!bBridgeMode ) pwd = 'pass' ; }
	else 		{ users_str += ',' ; }
...
if( bBridgeMode ){
	try {
		fs.statSync( 'v1/plugins.back' ) ;
	} catch(e){
		fs.move( 'v1/plugins' , 'v1/plugins.back' , function(){}) ;
	}
} else {
	try {
		fs.statSync( 'v1/plugins.back' ) ;
		fs.move( 'v1/plugins.back' , 'v1/plugins' , function(){} ) ;
	} catch(e){}
}